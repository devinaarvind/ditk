...
class RESIDE(RelationExtraction):
	...
	def run_epoch(self, sess, y_actual, y_pred, data, epoch, shuffle=True):
		"""
		...
		for step, batch in enumerate(self.getBatches(data, shuffle)):
			...
			if step % 10 == 0:
				...
				self.logger.info('Actual relation: {}'.format(y_actual))
				self.logger.info('Predicted relation: {}'.format(y_pred))
	...
	def fit(self, sess):
		...
		two_avg = (two_100 + two_200 + two_300)
		# P@All
